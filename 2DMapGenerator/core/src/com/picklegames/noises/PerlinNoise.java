...
public class PerlinNoise {
	...
	public double OctavePerlinS(double x, double y, double z, int octaves, double persistence) {
		double total = 0;
		double frequency = 1;
		double amplitude = 1;
		double maxValue = 0; // Used for normalizing result to 0.0 - 1.0
	...
	public float[][] OctavePerlin(int width, int height, double z, int octaves, double persistence) {
		float[][] totalNoise = new float[width][height];
		
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				totalNoise[x][y] = (float)OctavePerlinS(width,height,z,octaves,persistence);
			}
		}
