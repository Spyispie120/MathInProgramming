package com.picklegames.noises;
...
public class SimplexNoise { // Simplex noise in 2D, 3D and 4D
	...
	public float octavedNoise(float x, float y) {
		return (float) ((noise(x, y, 1f) * 1 + // Each of this lines is called
												// an "octave".
	...
	// Of course this is not hardcoded in the end, I just made that for making
	// it clear.
	// In 'reality' it would be implemented with the parameters "octaves" and
	// "roughness" and "scale":
	public static float octavedNoise(int width, int height, int octaves, float roughness, float scale) {
		float noiseSum = 0;
		...
		for (int octave = 0; octave < octaves; octave++) {
			noiseSum += noise(width * layerFrequency, height * layerFrequency) * layerWeight;
			layerFrequency *= 2;
	...
	public static float[][] generateOctavedSimplexNoise(int width, int height, int octaves, float roughness,
			float scale) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				float value = octavedNoise(x, y, octaves, roughness, scale);
				value = MathUtils.clamp(value, 0, 1);
				totalNoise[x][y] += value;
			}
		}
		return totalNoise;
	}
